    ...
    <form class="w-full px-3 sm:px-4 pt-3 mb-4">
      ...
      <div v-else>
        <div class="mb-4 flex overflow-x-scroll">
          <div v-for="item in frameData" :key="item.FrameID" @click="frameID = item.FrameID" :class="'rounded-lg mt-1 mb-4 mx-2 sm:mx-4 shrink-0 overflow-hidden shadow-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200 cursor-pointer transition-colors ' + (frameID == item.FrameID && 'ring-4 ring-indigo-500')">
            <div class="blcok">
              <!-- Image loading -->
        ...
        <div class="mb-4 flex overflow-x-scroll">
          <div v-for="item in lensData" :key="item.LensID" @click="lensID = item.LensID" :class="'rounded-lg mt-1 mb-4 mx-2 sm:mx-4 shrink-0 overflow-hidden shadow-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200 cursor-pointer transition-colors ' + (lensID == item.LensID && 'ring-4 ring-indigo-500')">
            <div class="blcok">
              <div class="px-2 py-1 my-auto mr-2">
...
// todo: вибір оправи + лінз нормально. + інші товари з хедера ловит
