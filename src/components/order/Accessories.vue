<template>
  <BaseCard>
    ...
    <form class="w-full px-3 sm:px-4 pt-3 mb-4">
      ...
      <div v-if="!data" style="height: 280px;"></div>
      <div v-else>
        ...
        <h1 class="text-2xl font-extrabold leading-9 tracking-tight sm:leading-10">{{ data.AccessoryName }}</h1>
        <p class="text-lg">
...
const getSunglasses = (apiSort) => {
  axios.get(`/api/accessories/get.php?id=${apiSort}`)
  .then(response => {
    data.value = response.data[0];
    //loading.value = false;
    console.log(data.value[AccessoryPrice.value] * quantity.value);
  })
  .catch(() => {
  ...
  axios({
    ...
    data: {
      order: {
        ...
        totalAmount: data.value.AccessoryPrice * quantity.value,
        accessoryID: data.value.AccessoryID,
        quantity: quantity.value,
        unitPrice: data.value.AccessoryPrice
      },
...
getSunglasses(route.query.accessory);
</script>