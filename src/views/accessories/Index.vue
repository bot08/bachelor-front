<template>
  ...
  <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-1 md:mb-0">
    <div v-for="item in data" :key="item.AccessoryName" class="rounded-lg mt-1 mb-3 md:mb-8 sm:mx-4 overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 transition-colors">
      <router-link :to="'/accessories/'+item.AccessoryID" class="flex">
        <!-- Image loading -->
        <LazyImage class="shrink-0">
          <template v-slot:image>
            <img class="w-28 h-28 mx-3 my-3 drop-shadow rounded-lg" :src="axios.defaults.baseURL+item.AccessoryImage" alt="icon">
          </template>
        ...
        <div class="px-1 py-1 my-auto mr-2">
          <div class="font-bold text-xl">{{ item.AccessoryName }}</div>
          <div class="text-lg">{{ item.AccessoryPrice }} грн</div>
          <div class="text-lg mb-2">{{ item.AccessoryManufacturer }}</div>
        </div>
      </router-link>
    </div>
  </div>
...
const getContent = (apiSort, hidePreloaders) => {
  ...
  axios.get(`/api/accessories/get.php?${apiSort}`)
  .then(response => {
